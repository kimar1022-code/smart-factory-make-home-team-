...
## ★ 도착 직후 조명 (9/6 새벽 실측)
- **직사광이 작업대에 들어옴**(손목캠·측면캠 모두 밝은 띠). 좌상 파란 기둥 점이 직사광 안에 들어가 노출 83·gain 16 에서도 하얗게 날아감(채도 2). 이건 카메라 설정으로 못 고침 → **블라인드/커튼으로 직사광 차단이 1순위**. 차단 후 `python3 color_lock.py check`.
- 손목캠은 지금 exp 83 + gain 16 으로 두었고 color_lock 저장값도 그것. 건강게이트는 노출 사다리(83~417)와 gain(16/64) 둘 다 자동으로 훑음. 저녁엔 gain 64 로 스스로 돌아감.
- 새카메라 노출 조절 불가(자동), 측면캠 자동. 둘 다 직사광 차단이면 기둥 점 안정(밤 실측 8/8).
