...
- 사이클 중 `measure_base()` 직접 호출 → 409. GOTO OBS 이동 중 서버 재기동 → 직후 409. 사이클 busy 중 손목캠 노출 변경(사다리와 충돌). 19:30 **`find_base_4pts()` 를 '오프라인 검출' 로 착각하고 상태 확인과 병렬로 띄움 → 로봇 탐색 상승 → 세 번째 409.** 규칙: **상태 확인 결과를 본 뒤에만** 로봇/카메라 명령, 같은 응답에 병렬 금지. `find_base_4pts/measure_base/goto_obs` 는 로봇을 움직인다. 베이스 단계 '❌ 어떤 노출/게인' 줄은 라운드 메시지지 정지가 아님.
