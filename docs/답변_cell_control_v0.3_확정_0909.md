...
| ✅ | RESUME 전 파지 재검증 (C8) | 검증됨 (단, 실기 판정기 배선은 미완 — 아래) |
| ✅ | 벽 든 채 RESUME → 되올라가 재측정 (C9) | 오케스트레이터 측 배선 완료 |
...
HELD 를 빠져나와 phase 를 다시 돌기 **직전**에 "부품이 아직 붙어 있는가"를 확인합니다.
결과는 세 가지이고, **세 번째를 통과로 위장하지 않는 것**이 이 설계의 핵심입니다.

| 결과 | 처리 |
|---|---|
| 붙어 있음 | 그대로 재개 |
| 떨어짐 | `CellError E201` → **FAULT**, Task `FAILED` (계약 §6 매핑대로) |
| **검증 수단 없음** | 진행하되 `WARN` + **`RESUME_UNVERIFIED` 이벤트** 발행 — "확인 못 했다"를 명시적으로 남깁니다 |

세 번째가 필요한 이유는 **ZK 진공에 피드백이 없기 때문**입니다(계약 §6 의 E201 을 셀이 검출 불가).
"검증했더니 괜찮더라"와 "검증할 방법이 없다"는 서버 입장에서 완전히 다른 정보인데,
전자로 뭉뚱그리면 없는 안전을 있다고 보고하는 셈이 됩니다.

`revalidate_on_resume` 파라미터로 끌 수 있고, 끄면 그 사실도 경고로 남깁니다.

**C9 — 벽을 든 채 재개**

`run.resume_from_hold` 플래그를 어댑터에 넘깁니다. 어댑터는 이 값이 `true` 면
**정렬 높이로 되올라가 다시 측정한 뒤** 내려갑니다(§4-1). sim 에서는 그 사실을 로그로 남깁니다.

```
[sim] 일시정지 재개 — INSERT 를 처음부터 다시 실행 (부품 든 채 → 실기라면 정렬 높이 복귀 후 재측정)
```

#### 실측 (도메인 99, sim)

```
C9  INSERT 중 즉시정지 → RESUME
    → "재개 전 그리퍼 파지 재검증 OK → INSERT 재시작"  → SUCCEEDED
V8-a 재검증 실패 주입 → cell_state=HELD, status=FAILED, error_code=E201
     detail='재개 전 흡착 파지 재검증 실패: …'
V8-b 검증 수단 없음   → WARN + RESUME_UNVERIFIED 이벤트, 진행 → SUCCEEDED
```

> 검증 중 결함 1건을 더 잡았습니다 — 재검증 실패가 `_boundary()` 안에서 발생하는데
> 그 호출이 `try/except CellError` **밖**에 있어, 액션 골이 **Result 없이 abort** 됐습니다
> (`status`·`error_code` 가 빈 채로 나감). 서버가 원인을 알 방법이 없는 최악의 실패 형태라
> 경계 호출을 감싸 정상적인 `FAILED` + `E201` 로 나가게 고쳤습니다.

#### 🔴 아직 실기에 못 붙인 것

`verify_held()` 의 **FR5 실기 판정기가 아직 위임 전**입니다. 실제 "벽을 물고 있는가" 판정은
저희 손목캠 검출기(`place_calc.held_wall_dots` 계열, 9/7~9/8 실기로 다듬은 것)가 하는데,
그 코드는 지금 별도 스택에 있고 오케스트레이터와 아직 연결되어 있지 않습니다.
지금은 `ok=None`("검증 수단 없음")을 돌려주므로 **`RESUME_UNVERIFIED` 이벤트가 뜹니다.**
연결 전까지는 그 이벤트를 "아직 미배선"으로 읽어 주시면 됩니다.

ZK 진공은 배선 문제가 아니라 **원리적으로 검증 수단이 없습니다.** 근본 해결은 압력 스위치 추가입니다.

#### 남은 순서

남은 것은 **`verify_held()` 의 FR5 실기 판정기 위임**과 **실기 검증**입니다.
